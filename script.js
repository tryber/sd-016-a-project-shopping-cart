...
function renewLocalStorageData(sku) {
  const storageProduct = JSON.parse(getSavedCartItems()).filter((product) => product.sku !== sku);
  localStorage.clear();
  if ((storageProduct).length !== 0) saveCartItems(JSON.stringify(storageProduct));
}
